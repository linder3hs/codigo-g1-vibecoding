...
jest.mock("../../pages", () => ({
  HomePage: () => (
    <div data-testid="home-page" className="min-h-screen bg-gradient-to-br from-slate-900 to-slate-800">
      <div className="container mx-auto px-4 py-8">
        <h1 className="text-3xl font-bold text-white mb-8">Todo VibeCoding - Home</h1>
        <p className="text-slate-300">Gestiona tus tareas con estilo minimalista</p>
      </div>
    </div>
  ),
  CreateTodoPage: () => (
    <div data-testid="create-todo-page" className="min-h-screen bg-gradient-to-br from-slate-900 to-slate-800">
      <div className="container mx-auto px-4 py-8">
        <h1 className="text-3xl font-bold text-white mb-8">Crear Nueva Tarea</h1>
        <p className="text-slate-300">Formulario de creación con glassmorphism</p>
      </div>
    </div>
  ),
}));
...
jest.mock("../ErrorPage", () => ({
  ErrorPage: () => (
    <div data-testid="error-page" className="min-h-screen bg-gradient-to-br from-slate-900 to-slate-800">
...
describe("AppRouter - Minimalist UI Routing", () => {
  ...
  it("renders HomePage with minimalist design for root route", () => {
    ...
    expect(screen.getByText("Gestiona tus tareas con estilo minimalista")).toBeInTheDocument();
    
    // Verify CSS classes for minimalist design
    expect(homePage).toHaveClass("min-h-screen", "bg-gradient-to-br", "from-slate-900", "to-slate-800");
  });
  ...
  it("renders CreateTodoPage with glassmorphism design for /crear-todo route", () => {
    ...
    expect(screen.getByText("Formulario de creación con glassmorphism")).toBeInTheDocument();
    
    // Verify CSS classes for minimalist design
    expect(createTodoPage).toHaveClass("min-h-screen", "bg-gradient-to-br", "from-slate-900", "to-slate-800");
  });
  ...
  it("renders ErrorPage with minimalist design for unknown routes", () => {
    ...
    expect(errorPage).toHaveClass("min-h-screen", "bg-gradient-to-br", "from-slate-900", "to-slate-800");
  });
